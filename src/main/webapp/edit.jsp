...
<%
	...
	if (null != request.getParameter("id")) {
		entryBean.isEntryExists(Integer.parseInt(request.getParameter("id")));
	}
	else {
		entryBean.isEntryExists(0);
	}
	
	if (null != request.getParameter("UPDATE") && null != entryBean.getEntry()) {
...
<body>
 ...
 <div class="content block">
  <c:choose>
   <c:when test="${null == entryBean.entry}">
    <table class="information">
     <tr>
      <td>Внутренняя ошибка: требуемая запись отсутствует</td>
     </tr>
     <tr>
   ...
   <c:when test="${null != entryBean.errors && entryBean.errors.size() == 0}">
    <table class="information">
     <tr>
      <td>Запись была успешно изменена!</td>
     </tr>
     <tr>
      <td><a href="${pageContext.request.contextPath}/index.jsp" class="link">Вернуться на главную</a></td>
     </tr>
   ...
   <c:otherwise>
    ...
    <input type="hidden" name="id" value="${entryBean.entry.id}">
    <table class="entryEdit">
     ...
     <tr class="buttons">
      <td><input type="submit" name="UPDATE" value="Изменить запись"></td>
